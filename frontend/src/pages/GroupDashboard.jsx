...
const GroupDashboard = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="text-[#6C4AB6] font-bold text-lg mb-1 px-5 mt-3">Transactions</div>
        {group.transactions.length === 0 && (
          <div className="text-[#2D2D2D] text-sm px-5 ">No transactions yet.</div>
        )}
    {group.transactions.length > 0 && (
      <div className="px-6 py-4 bg-[#E9E9FE] rounded-2xl shadow-lg mx-4 mt-2">
  {group.transactions.map((txn) => (
    ...
    >
      <div className="flex items-start gap-3">
        {/* Transaction Icon */}
        {/* <div className={`p-2 rounded-lg ${txn.amount >= 0 ? 'bg-green-100' : 'bg-red-100'}`}>
          {txn.amount >= 0 ? (
            <span className="text-green-600 text-xl">↑</span> // Income icon
          ) : (
            <span className="text-red-600 text-xl">↓</span> // Expense icon
          )}
        </div> */}
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="font-semibold text-[#2D2D2D]">{txn.description}</div>
              <div className="text-xs text-gray-500 mt-1">
                {new Date(txn.createdAt).toLocaleDateString('en-US', { 
                  month: 'short', 
                  day: 'numeric' 
                })} &middot; Paid by <span className="font-medium">{txn.payer.username}</span>
              </div>
            </div>
            <div className={`font-bold text-lg ${txn.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {txn.amount >= 0 ? '+' : ''}{txn.currency} {Math.abs(txn.amount).toLocaleString()}
            </div>
          </div>
            ...
            {txn.splitDetails.map((split) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="font-medium text-[#2D2D2D]">{split.user.username}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    split.isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                  }`}>
      ...
      {/* Add Expense Button */}
      <button className="fixed bottom-24 right-6 bg-[#6C4AB6] hover:bg-[#8D6BD9] text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2 text-lg font-semibold z-10">
        <svg
          width="24"
          height="24"
          fill="none"
